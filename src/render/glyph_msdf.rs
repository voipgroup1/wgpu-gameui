...
use fdsm::bezier::{Point, Segment, scanline::FillRule};
use fdsm::generate::generate_msdf;
use fdsm::render::correct_sign_msdf;
use fdsm::shape::{ColoredContour, Shape, Contour};
...
pub fn generate_glyph_msdf(
    ...
    let shape = load_shape_from_face(face, glyph)?;
...
fn load_shape_from_face(face: &Face, glyph_id: GlyphId) -> Option<Shape<Contour>> {
