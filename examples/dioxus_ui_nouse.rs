...
static mut web_app: LazyLock<WebApp> =  LazyLock::new(|| WebApp {
    window: None,
    gpu: None,
...
static mut tmp_app_state:LazyLock<Rc<RefCell<AppTmpState>>> = LazyLock::new (|| Rc::new(RefCell::new(AppTmpState{
    surface: None,
...
pub async fn run_wgpu_app() {
    ...
    #[cfg(target_arch="wasm32")]
    unsafe {
        event_loop.spawn_app(&mut *web_app);
    }
    
    #[cfg(not(target_arch="wasm32"))]
    unsafe {
        event_loop.run_app(&mut *web_app).expect("run webapp");
    }
}
...
struct AppTmpState<'a,'b,'c> {
    surface: Option<wgpu::Surface<'a>>,
    instance: wgpu::Instance,
    adapter: Option<wgpu::Adapter>,
    device_que: Option<(wgpu::Device, wgpu::Queue)>,
    request_adapter: Option<wgpu::RequestAdapterOptions<'a,'b>>,
    device_descriptor: wgpu::DeviceDescriptor<'c>,
}
...
impl ApplicationHandler for WebApp {
    ...
    fn resumed(&mut self, event_loop: & ActiveEventLoop) {
        ...
        // Set up wgpu.
        //let tmp_state = Rc::new(RefCell::new (tmp_app_state));
        let tmp_state_0 = unsafe { tmp_app_state.clone() };
        let mut state_0 = tmp_state_0.borrow_mut();
        state_0.surface = state_0.instance
            .create_surface(window.clone())
            .expect("create surface").into();

        state_0.request_adapter = wgpu::RequestAdapterOptions {
                power_preference: wgpu::PowerPreference::default(),
                compatible_surface: Some(&state_0.surface.as_ref().unwrap()),
        ...
        let tmp_state_1 = unsafe { tmp_app_state.clone() };
        js_sys::futures::spawn_local(async move {
            let mut state_1 = tmp_state_1.borrow_mut();
            state_1.adapter = state_1.instance.request_adapter(&state_1.request_adapter.as_ref().unwrap()).await
            .expect("request adapter").into();
        });

        let tmp_state_2 = unsafe { tmp_app_state.clone() };
        /* 
        ...
        */
        js_sys::futures::spawn_local(async move {
            let mut state_2 = tmp_state_2.borrow_mut();
            state_2.device_que = state_2.adapter.as_ref().unwrap().request_device(&wgpu::DeviceDescriptor {
                label: Some("hello_ui device"),
                required_features: wgpu::Features::empty(),
                required_limits: wgpu::Limits::downlevel_webgl2_defaults(),
                memory_hints: Default::default(),
        ...
        */
        let tmp_state_3 = unsafe { tmp_app_state.clone() };
        let state_3= tmp_state_3.borrow_mut();
        let size = window.inner_size();
        let surface_caps = state_3.surface.as_ref().unwrap().get_capabilities(&state_3.adapter.as_ref().unwrap());
        ...
        state_3.surface.as_ref().unwrap().configure(&state_3.device_que.as_ref().unwrap().0, &config);
        ...
        let mut ui = UiRenderer::new(&state_3.device_que.as_ref().unwrap().0, &state_3.device_que.as_ref().unwrap().1, format, font_system);
        ...
        self.gpu = Some(Rc::new(RefCell::new(Gpu {
            //surface,
            //device: state_3.device_que.as_ref().unwrap().0,
            //queue: state_3.device_que.as_ref().unwrap().1,
            config,
            ui,
            icon_sprite,
            nine_slice_id,
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::Resized(size) => {
                ...
                let tmp_state_0 = gpu.tmp_state.clone();
                let mut state_0 = tmp_state_0.borrow_mut(); 
                state_0.surface.as_ref().wrap().configure(&state_0.device_que.as_ref().unwrap().0, &gpu.config);
                gpu.ui
                    .resize(&state_0.device_que.as_ref().unwrap().1, gpu.config.width, gpu.config.height);
                window.request_redraw();
            }
            ...
            WindowEvent::RedrawRequested => {
                ...
                let frame = match state_0.surface.as_ref().unwrap().get_current_texture() {
                    wgpu::CurrentSurfaceTexture::Success(f) => f,
                    wgpu::CurrentSurfaceTexture::Suboptimal(_) => {
                        state_0.surface.as_ref().unwrap().configure(&state_0.device_que.as_ref().unwrap().0, &gpu.config);
                        return;
                    },
                    _ => { return; }
                };
                ...
                let mut encoder =
                    state_0.device_que.as_ref().unwrap().0
                ...
                gpu.ui.render_layers(
                    &state_0.device_que.as_ref().unwrap().0,
                    &state_0.device_que.as_ref().unwrap().1,
                    &mut encoder,
                    &view,
                ...
                state_0.device_que.as_ref().unwrap().1.submit(Some(encoder.finish()));
                state_0.device_que.as_ref().unwrap().1.present(frame);
