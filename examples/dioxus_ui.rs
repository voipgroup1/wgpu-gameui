...
thread_local! {   
    static web_app: RefCell<Option<WebApp>> =  RefCell::new(None);
}
...
pub async fn run_wgpu_app() {
    ...
    let mut app = web_app.with(|s| s.borrow_mut().take());

    #[cfg(target_arch="wasm32")]
    use winit::platform::web::EventLoopExtWebSys;
    #[cfg(target_arch="wasm32")]
    match app.as_mut() {
        Some(w) => {event_loop.run_app(w).expect("run webapp");}
        _ => {},
    }
...
impl WebApp {
    async fn resumed_async(&mut self) {
...
impl ApplicationHandler for WebApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if self.window.is_some() {
                web_sys::console::log_1(&"baaa".into()); 
            return;
        }
        #[cfg(target_arch="wasm32")]
        ...
        js_sys::futures::spawn_local( async move {
            unsafe { (*ptr).resumed_async().await; }
        });
        /*
        let app = web_app.with(|s| s.borrow_mut().take());
        js_sys::futures::spawn_local(async move {
            if let Some(mut app)  = app {
                app.resumed_async().await;
                web_app.with(|s| {
